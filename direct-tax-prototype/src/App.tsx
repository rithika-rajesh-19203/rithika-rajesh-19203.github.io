...
interface RegionRecord extends TaxReturnSettingsForm {
  id: string;
  avalaraFlow: Exclude<AvalaraFlow, null>;
    ...
    <div className="space-y-5">
      <div className="bg-white border border-[#ebeaf1] rounded-lg px-6 py-5">
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-[11px] text-[#6d7188] leading-5 mt-1">
              Review and digitally sign your Form POA (Power of Attorney) after connecting Avalara. Tax return configuration is unlocked only after this step is completed.
            </p>
          </div>
          <span
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                isComplete ? "bg-green-500" : connected ? "bg-amber-500" : "bg-gray-400"
              }`}
            />
            {isComplete ? "Completed" : connected ? "Ready to sign" : "Connect Avalara first"}
          </span>
          {connected && (
            <button
              type="button"
              onClick={() => setCollapsed((current) => !current)}
              className="inline-flex items-center gap-1 text-[11px] text-[#0D81FD] font-medium hover:underline"
              style={{ fontFamily: "'Inter:Medium', sans-serif" }}
            >
              {collapsed ? "Show section" : "Hide section"}
              <svg
                viewBox="0 0 24 24"
                className={`w-3 h-3 fill-current transition-transform ${collapsed ? "" : "rotate-180"}`}
              >
        ...
        {collapsed ? (
          ...
          <div className="border border-dashed border-[#d7d9e4] rounded-lg px-6 py-8 flex items-center justify-between gap-4 bg-gray-50/60">
            <div>
              <h3 className="text-[13px] font-medium text-gray-900" style={{ fontFamily: "'Inter:Medium', sans-serif" }}>
                Connect Avalara to begin FPOA signing
              </h3>
              <p className="text-[11px] text-[#6d7188] mt-1">
                Sign up with Avalara first, then return here to sign the FPOA and unlock tax return configuration.
              </p>
            </div>
            <button
              onClick={onConnect}
              className="bg-[#0D81FD] text-white text-[12px] px-4 py-2 rounded hover:bg-blue-600 transition-colors flex-shrink-0"
              style={{ fontFamily: "'Inter:Medium', sans-serif" }}
            >
              Connect to Avalara
            </button>
...
function QuestionnaireSection({
  ...
  return (
    <div className="bg-white border border-[#ebeaf1] rounded-lg px-6 py-5 space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-[#6d7188] leading-5">
            Answer these questions on demand so we can suggest nexus regions and tax forms for every business you configure below.
          </p>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                allAnswered ? "bg-green-500" : hasSavedAnswers ? "bg-blue-500" : "bg-amber-500"
              }`}
            />
            {allAnswered
          ...
          <button
            ...
            style={{ fontFamily: "'Inter:Medium', sans-serif" }}
          >
            {expanded ? "Hide questionnaire" : "Answer questionnaire"}
      ...
      {expanded && (
        <>
          ...
          <div className="flex items-center justify-between gap-3 pt-1">
            <p className="text-[10px] text-gray-400">
              Your responses will be used to suggest forms when you launch filing setup for a region.
            </p>
          ...
          <div key={row.id} className="border border-gray-200 rounded-lg p-4 bg-white">
            ...
            <div className="ml-10.5 flex flex-wrap gap-1.5">
              {row.forms.map((form) => (
                <span
                  key={form}
                  className="inline-flex items-center gap-1 bg-gray-50 border border-gray-200 text-[10px] text-gray-600 px-2 py-0.5 rounded"
                >
                  {form}
                  <button
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-[#6d7188] mt-1">
            Add each business region here, store its filing details, and maintain the suggested nexus and tax forms from your questionnaire answers in one table.
          </p>
        </div>
      </div>
      ...
      {!enabled ? (
        <div className="bg-white border border-dashed border-[#d7d9e4] rounded-lg px-6 py-12 text-center">
          ...
          <h3 className="text-[14px] font-medium text-gray-900" style={{ fontFamily: "'Inter:Medium', sans-serif" }}>
            Complete FPOA signing to unlock tax return setup
          </h3>
          <p className="text-[11px] text-[#6d7188] mt-2">
            Once the FPOA is signed successfully, you can configure tax returns for multiple states and store each setup in this table.
          </p>
        </div>
      ) : regions.length === 0 ? (
        <div className="bg-white border border-dashed border-[#d7d9e4] rounded-lg px-6 py-12 text-center">
          ...
          <p className="text-[11px] text-[#6d7188] mt-2 mb-5">
            Start by adding a business region and storing the details needed for tax return filing.
          </p>
          <button
            onClick={onAddRegion}
        ...
        <div className="bg-white border border-[#ebeaf1] rounded-lg overflow-hidden">
          ...
          <div className="grid grid-cols-[1fr_1.1fr_1fr_0.8fr_0.8fr_0.8fr_1fr] gap-4 px-6 py-3 bg-gray-50 border-b border-gray-100 text-[10px] tracking-wide uppercase text-gray-400">
            <span>Business region</span>
            <span>Legal entity</span>
            <span>Account number</span>
            <span>Suggested nexus</span>
            <span>Suggested forms</span>
            <span>Status</span>
            <span>Actions</span>
          </div>
          ...
          <div className="divide-y divide-gray-100">
            {regions.map((region) => {
              ...
              return (
                <div key={region.id} className="grid grid-cols-[1fr_1.1fr_1fr_0.8fr_0.8fr_0.8fr_1fr] gap-4 px-6 py-4 text-[12px] text-gray-700 items-start">
                  <div>
                  ...
                  <div>
                    <div>{region.legalEntityName}</div>
                    <div className="text-[10px] text-gray-400 mt-1">{region.efileUsername}</div>
                  </div>
                  <div>{region.salesTaxAccountNumber}</div>
                  <div>
                    <div>{suggestedRows.length} region{suggestedRows.length !== 1 ? "s" : ""}</div>
                    <div className="text-[10px] text-gray-400 mt-1">Suggested from questionnaire</div>
                  </div>
                  <div>
                    <div>{formsCount} form{formsCount !== 1 ? "s" : ""}</div>
                    <div className="text-[10px] text-gray-400 mt-1">Auto-suggested</div>
                  </div>
                  <div>
                    <span
                      className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border ${
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="text-[17px] font-medium text-gray-900" style={{ fontFamily: "'Inter:Medium', sans-serif" }}>
            Filing setup for {region.state}
          </h2>
          <p className="text-[11px] text-[#6d7188] mt-1">
            Review the suggested nexus regions and tax forms for this business configuration and save any state-specific updates.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#ebeaf1] rounded-lg px-6 py-5 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-[#6d7188] mt-1">
            {region.state} • {region.salesTaxAccountNumber} • {region.efileUsername}
          </p>
        </div>
        {!connected && (
          <button
      ...
      {!connected && (
        <div className="bg-white border border-dashed border-[#d7d9e4] rounded-lg px-6 py-10 text-center">
          <h3 className="text-[14px] font-medium text-gray-900" style={{ fontFamily: "'Inter:Medium', sans-serif" }}>
            Connect Avalara to continue
          </h3>
          <p className="text-[11px] text-[#6d7188] mt-2 mb-4">
            Sign up with Avalara first, then continue with nexus and tax form setup for this region.
          </p>
          <button
            onClick={onConnect}
            className="bg-[#0D81FD] text-white text-[12px] px-4 py-2 rounded hover:bg-blue-600 transition-colors"
...
export default function App() {
  ...
  const fpoaCompleted = fpoaStatus === "processing";

  useEffect(() => {
    if (fpoaStatus !== "signing") {
      return;
    }

    const timer = window.setTimeout(() => setFpoaStatus("processing"), 2200);
  ...
  function handleSettingsSave(form: TaxReturnSettingsForm) {
    setShowSettingsForm(false);
    setRegions((prev) => [
      ...prev,
      {
        id: `${form.state}-${prev.length + 1}`,
        ...form,
        avalaraFlow: "existing",
    ...
    setToastMessage("Business region saved. Launch filing setup from the table.");
  }

  function handleConnect(flow: Exclude<AvalaraFlow, null>) {
    setShowAvalaraModal(false);
    setConnected(true);
    if (selectedRegionId) {
      setRegions((prev) =>
        prev.map((region) =>
          region.id === selectedRegionId ? { ...region, avalaraFlow: flow } : region
        )
      );
    }
    setToastMessage("Avalara is connected. You can now sign the FPOA.");
  }

  function handleFpoaSign() {
    if (!connected || fpoaCompleted) {
      return;
    }
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <div className="flex items-center gap-4">
                  <button className="text-[12px] text-gray-600 hover:text-gray-800">Disable Tax</button>
              ...
              <div className="bg-white border border-[#ebeaf1] rounded-lg px-6 py-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-[#6d7188] leading-5 mt-0.5">
                      Turn on direct tax filing and connect Avalara before configuring tax return filing for each business region.
                    </p>
                  </div>
                ...
                {taxEnabled && (
                  <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-start">
                    <button
                      onClick={() => setShowAvalaraModal(true)}
                      className="text-[11px] text-[#0D81FD] font-medium hover:underline flex items-center gap-1"
                      style={{ fontFamily: "'Inter:Medium', sans-serif" }}
                    >
                      ...
                      {connected ? "Manage AVALARA connection" : "Sign up with AVALARA"}
                    </button>
                  </div>
                )}
              </div>
              ...
              {taxEnabled && (
                <div className="space-y-6 mt-6">
                  ...
                  <OverviewFpoaSection
                    connected={connected}
                    fpoaStatus={fpoaStatus}
                    onConnect={() => setShowAvalaraModal(true)}
                    onSign={handleFpoaSign}
                  />
                  {fpoaCompleted && (
                    <ConfigureTaxReturnSection
                      regions={regions}
                      enabled={fpoaCompleted}
                      onAddRegion={() => setShowSettingsForm(true)}
